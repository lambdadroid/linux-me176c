...
DECLARE_PCI_FIXUP_ENABLE(PCI_VENDOR_ID_INTEL, 0x0f35,
...
static void quirk_byt_ush_suspend(struct pci_dev *dev)
{
	...
	dev_err(&dev->dev, "USH suspend quirk\n");
	...
	pci_read_config_dword(dev, PCI_USH_SSCFG1, &value);
	dev_err(&dev->dev, "PCI B0 reg (SSCFG1) = 0x%x\n", value);
}
DECLARE_PCI_FIXUP_SUSPEND(PCI_VENDOR_ID_INTEL, 0x0f35,
...
static void quirk_byt_ush_resume(struct pci_dev *dev)
{
	...
	dev_err(&dev->dev, "USH resume quirk\n");
	pci_read_config_dword(dev, PCI_USH_SSCFG1, &value);
...
DECLARE_PCI_FIXUP_RESUME(PCI_VENDOR_ID_INTEL, 0x0f35,
