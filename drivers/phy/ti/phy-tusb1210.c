...
static void tusb1211_detect_charger_type(struct tusb1210 *tusb)
{
	...
	if (tusb->phy->power_count == 0) {
		ret = tusb1210_power_on(tusb->phy);
		if (ret) {
			dev_err(&tusb->phy->dev,
				"Failed to power on PHY for charger detection: %d\n", ret);
			ret = 0; /* Detect as SDP */
			goto err;
		}
	}
...
err:
	...
	if (tusb->phy->power_count == 0) {
		dev_err(&tusb->phy->dev, "Power off again\n"); // TODO

		/* Power off PHY again */
		tusb1210_power_off(tusb->phy);
	} else {
		dev_err(&tusb->phy->dev, "Reset and restore mode\n"); // TODO
