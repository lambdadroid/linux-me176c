...
void bq24192_notify_cable_type(int usb_state) {
	if (chip_extern) {
		bq24192_cable_callback(usb_state);
	} else {
		// We might not be initialized when this is called
			...
			const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->otg_wrkr, otg_worker); // TODO
